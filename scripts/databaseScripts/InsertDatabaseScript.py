...
def drop_constrains_from_all_tables():
    ...
    file.write("ALTER TABLE \"User\" DROP CONSTRAINT IF EXISTS \"PK_User\" CASCADE;\n")
    file.write("ALTER TABLE \"Client\" DROP CONSTRAINT IF EXISTS \"PK_Client\" CASCADE;\n")
    file.write("ALTER TABLE \"Client\" DROP CONSTRAINT IF EXISTS \"FK_Client_UserId\" CASCADE;\n")
    file.write("ALTER TABLE \"Vehicle\" DROP CONSTRAINT IF EXISTS \"PK_Vehicle\" CASCADE;\n ")
    file.write("ALTER TABLE \"Vehicle\" DROP CONSTRAINT IF EXISTS \"FK_Vehicle_UserId\" CASCADE;\n")
    ...
    file.write("ALTER TABLE \"Incidents\" DROP CONSTRAINT IF EXISTS \"FK_Incidents_UserId\" CASCADE;\n")
    ...
    file.write("ALTER TABLE \"VehicleRent\" DROP CONSTRAINT IF EXISTS \"FK_VehicleRent_UserId\" Cascade;\n")
...
def add_constrains_to_all_tables():
    ...
    file.write("ALTER TABLE \"User\" ADD CONSTRAINT \"PK_User\" PRIMARY KEY(\"Id\");\n")
...
def insert_into_user():
    ...
    print("STARTING INSERTING INTO USER AT TIME: ", Fore.YELLOW +  str(datetime.datetime.now()))
    ...
    for index in range(number_of_users):
        ...
        user = User(uid,username,password,bio,location,birthday,gender,marital_status)
        user_batches += user.get_user_string() + ","
        user_profile_batches += user.get_user_profile_string() + ","
        if (index + 1) % 1000 == 0:
            file.write(str(f"INSERT INTO \"User\"(\"Id\", \"Name\", \"Password\") VALUES {user_batches[:-1]};\n"))
            file.write(str(f"INSERT INTO \"UserProfile\"(\"UserId\", \"Bio\", \"Location\",\"Birthday\",\"Gender\",\"MaritalStatus\") VALUES {user_profile_batches[:-1]};\n"))
            user_batches = ""
            user_profile_batches = ""

    file.close()
    print(Fore.WHITE + "STOP INSERTING INTO USER AT: ",Fore.YELLOW + str(datetime.datetime.now()), Fore.GREEN + "\u2713")
...
def insert_into_client():
    ...
    for index in range(number_of_clients):
        ...
        client = Client(cid,name,card_number,cnp,birthday,nationality,user_id)
        batches += str(client) + ","
        if (index + 1) % 1000 == 0:
            file.write(str(f"INSERT INTO \"Client\"(\"Id\",\"Name\",\"CardNumber\",\"CNP\",\"Birthday\",\"Nationality\",\"UserId\") VALUES {batches[:-1]};\n"))
            batches = ""    
...
def insert_into_vehicles():
    ...
    for index in range(number_of_vehicles):
        ...
        vehicle = Vehicle(vid,brand,horse_power,car_plate,number_of_seats,engine_capacity,fabrication_date,user_id)
        batches += str(vehicle) + ","
        if (index + 1) % 10 == 0:
            file.write(str(f"INSERT INTO \"Vehicle\"(\"Id\",\"Brand\",\"HorsePower\",\"CarPlate\",\"NumberOfSeats\",\"EngineCapacity\",\"FabricationDate\",\"UserId\") VALUES {batches[:-1]};\n"))
            batches = ""
    file.close()
...
def insert_into_incidents():
    ...
    for index in range(number_of_incidents):
        ...
        incident = Incident(iid, vehicle_id, location, description, cost, incident_date, user_id)
        batches += str(incident) + ","
        if (index + 1) % 1000 == 0:
            file.write(str(f"INSERT INTO \"Incidents\"(\"Id\",\"VehicleId\",\"Location\",\"Description\",\"Cost\",\"WhenHappend\",\"UserId\") VALUES{batches[:-1]};\n"))
            batches = ""
...
def insert_into_rents():
    ...
    for index in range(number_of_rents):
        ...
        rent = VehicleRent(rid,client_id,vehicle_id,start_date,end_date,total_cost,comments,user_id)
        batches += str(rent) + ","
        rent_ids[rid] = 1
        if (index + 1) % 1000 == 0:
            file.write(str(f"INSERT INTO \"VehicleRent\"(\"Id\",\"ClientId\",\"VehicleId\",\"StartDate\",\"EndDate\",\"TotalCost\",\"Comments\",\"UserId\") VALUES {batches[:-1]};\n"))
            batches = ""
